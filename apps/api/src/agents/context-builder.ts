...
const TEST_CONTRACT_LAYERS = new Set([2, 3]);

/**
 * @description Reads test-contracts.md generated by the seed-agent (Layer 1.5).
 * Returns empty string if file doesn't exist yet.
 */
async function readTestContracts(projectDir: string): Promise<string> {
  try {
    return await fs.readFile(path.join(projectDir, 'test-contracts.md'), 'utf8');
  } catch {
    return '';
  }
}
...
export async function buildTaskPrompt(opts: ContextOptions): Promise<string> {
  ...
  // 1.8. Read test-contracts.md (seed-agent output) for backend-agent (L2) and frontend-agent (L3)
  let testContractsSection = '';
  if (currentLayer !== undefined && TEST_CONTRACT_LAYERS.has(currentLayer)) {
    const contracts = await readTestContracts(projectDir);
    if (contracts) {
      testContractsSection = `\n\n## Test Contracts (generado por seed-agent)\n${contracts}`;
    }
  }
  ...
  return taskTemplate
    .replace('{{SPEC}}', specContent + specArtifactsSection + executionPlanSection + testContractsSection + memorySection + criteriaMapSection + testMappingSection)
